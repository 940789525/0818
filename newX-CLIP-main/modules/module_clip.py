...
class MvTransformer(nn.Module):
    def __init__(self, mode, input_resolution: int, patch_size: int, width: int, heads: int, output_dim: int,):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        if mode=="res":
            self.conv1 = nn.Conv2d(in_channels=3, out_channels=width, kernel_size=patch_size, stride=patch_size, bias=False)
        if mode=="mv":
            self.conv1 = nn.Conv2d(in_channels=2, out_channels=width, kernel_size=patch_size, stride=patch_size,
                                   bias=False)
        scale = width ** -0.5
        self.class_embedding = nn.Parameter(scale * torch.randn(width))
        self.positional_embedding = nn.Parameter(scale * torch.randn((input_resolution // patch_size) ** 2 + 1, width))
        ...
        self.transformer = Transformer(width, 3, heads)
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        # self.Adapter = Adapter(vision_width).to(self.dtype)
    ...
    def encode_image(self, image, return_hidden=False, video_frame=-1):
        ...
        # x = self.Adapter(hidden)
        #
        # ratio = 0.2
        # hidden = ratio * x + (1 - ratio) * hidden
    ...
    def encode_mv(self, mv, return_hidden=False, video_frame=-1):
        hidden = self.Mvisual(mv.type(self.dtype), video_frame=video_frame)
        hidden = self.Mvisual.ln_post(hidden) @ self.Mvisual.proj
    ...
    def encode_res(self, res, return_hidden=False, video_frame=-1):
        hidden = self.Rvisual(res.type(self.dtype), video_frame=video_frame)
        hidden = self.Rvisual.ln_post(hidden) @ self.Rvisual.proj

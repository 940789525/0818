...
class MSRVTT_DataLoader(Dataset):
    ...
    def _get_rawvideo(self, choice_video_ids):
        ...
        res = np.zeros((len(choice_video_ids), self.max_frames, 1, 3,
                        self.rawVideoExtractor.size, self.rawVideoExtractor.size), dtype=np.float)
        mv = np.zeros((len(choice_video_ids), self.max_frames, 1, 2,
                       self.rawVideoExtractor.size, self.rawVideoExtractor.size), dtype=np.float)

        for i, video_id in enumerate(choice_video_ids):
            # Individual for YoucokII dataset, due to it video format
            # print(f"Processing video ID: {video_id}")  # 打印当前处理的视频ID
            ...
            if self.max_frames < raw_res_slice.shape[0]:
                res_slice = raw_res_slice[:self.max_frames, ...]
            else:
                res_slice = raw_res_slice
            if self.max_frames < raw_mv_slice.shape[0]:
                mv_slice = raw_mv_slice[:self.max_frames, ...]
            else:
        ...
        return video, video_mask, res, mv
    ...
    def __getitem__(self, idx):
        ...
        return pairs_text, pairs_mask, pairs_segment, video, res, mv, video_mask

...
class UpsampleUpdatingModel2(nn.Module):
    ...
    def forward(self, i_features, p_motions, p_features):
        ...
        i_features = i_features.sum(dim=(2, 3))  # (bn gop) c
...
class XCLIP(CLIP4ClipPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, video, res, mv, video_mask=None):
        ...
        Pvisual_output = self.get_pvisual_output(visual_output, video_mask, res, mv)
        ...
        if self.training:
            ...
            sim_matrix = 0.9 * sim_matrix + 0.1 * sim_matrix1
    ...
    def get_pvisual_output(self, visual_output, video_mask, res, mv):
